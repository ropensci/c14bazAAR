...
c14.data <- get_c14data(databases = "all") %>% 
  #calibrate() %>%
  #mark_duplicates() %>%
  #classify_material() %>%
  #finalize_country_name() %>%
...
```{r figure, echo=FALSE, fig.align='center', dpi=300, out.width='100%'}
# map
c14.map <- ggplot(land) + 
...
plot_grid(c14.map, c14.spd, 
               labels = "AUTO",
               align = "h", 
          rel_widths = c(2, 1.5))
#ggsave("../c14bazAAR.png", width = 12, height = 8)
```
